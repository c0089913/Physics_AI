...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	if (a.type == GameObject::Type::REWARD || b.type == GameObject::Type::REWARD)
		return;
	...
	//friction
	float coeff = (physA->GetFriction() * physB->GetFriction());
	Vector3 normal = (contactVelocity - (p.normal * (Vector3::Dot(contactVelocity, p.normal)))).Normalised();

	Vector3 fObjA = Vector3::Cross(physA->GetInertiaTensor() * Vector3::Cross(relativeA, normal), relativeA);
	Vector3 fObjB = Vector3::Cross(physB->GetInertiaTensor() * Vector3::Cross(relativeB, normal), relativeB);

	float imp = (-(coeff * (Vector3::Dot(contactVelocity, normal)))) / (totalMass + Vector3::Dot(fObjA + fObjB, normal));
	Vector3 frictionImpulse = normal * imp;
